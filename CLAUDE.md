...
- TRPGシステム固有のGM呼称は、文脈から対象システムを一意に決められる箇所だけで使う。UI上1ワードしか使えない共通箇所では、新しい総称を作らず既定の「GM」に統一する
- 最初の管理者は `bin/rails admin:grant EMAIL=... NAME=...` で作る。管理画面からは作れない
- 一覧の既定の並びは `scenarios.position`。「シナリオの管理」で行をドラッグして決め、公開側では「GMのおすすめ順」と呼ぶ
- おすすめ度は並び順の材料ではなくなった。列と値は残っているが、どの画面にも出さない。切り戻す必要がなくなったら列ごと落とす
...
- 編集可能な要素には詳細画面を用意し、詳細画面と編集画面を相互にリンクする
- 一覧には詳細と編集の両方への導線を置く。作成・更新後は対象の詳細へ、削除後は一覧へ遷移する
