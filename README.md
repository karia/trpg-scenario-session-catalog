...
シナリオ情報は誰でも閲覧できる。
セッション情報は Discord 認証でログインし、参加者と同じグループに所属しているユーザーだけが閲覧できる。
...
Discord ログインや Google 連携を手元で試すときは、利用する provider の client ID と client secret が要る。
...
実装前にそこを読み、判断を変えるときは新しい ADR を先に書く。
既存の ADR は決定時点の記録として残し、変えるのはステータスだけにする。
置き換えた ADR のステータスは、全体なら「置き換え済み」、一部なら「一部置き換え」とし、後継の ADR へのリンクを添える。
