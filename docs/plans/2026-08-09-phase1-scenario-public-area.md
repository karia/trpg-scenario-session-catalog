...
| オススメ度 | `Scenario#recommendation`（1 から 5、NULL 可） |
...
- オススメ度には空欄と「回したことない」の 2 種類がある。前者は `recommendation` の NULL、後者は `Scenario#run_by_owner` の false で表し、両者を区別する
- キャラシ提出期限は、前日、前々日、1 週間前、不要、備考参照の 5 つで観測値をすべて覆える。列挙型にする
...
- `app/policies/scenario_policy.rb`
...
5. 画面を組む。ここで frontend-design スキルを使う
6. ジャケット画像を Active Storage で添付し、一覧用の variant を用意する
7. Avo のリソースを定義し、シナリオを登録できるようにする
8. meta-tags でタイトルと description、OGP を出す
9. シナリオ詳細に JSON-LD を出す
10. sitemap_generator でサイトマップを生成し、`robots.txt` から参照する
11. 解析タグと広告タグを部分テンプレートに切り出す
...
- Lighthouse の SEO スコアが 90 以上になる
- 準備情報が、この時点ではどのユーザーにも表示されない

## 詰まりそうな箇所

**Avo の認可は Phase 2 の Pundit に依存する**。
Phase 1 の時点では管理者の概念がまだない。
Avo のルーティングを本番環境では読み込まない形にしておき、Phase 2 で管理者に開く。
