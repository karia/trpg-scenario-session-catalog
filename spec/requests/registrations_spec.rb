...
RSpec.describe "Registrations" do
  it "explains the registration steps and provides Discord sign-in" do
    get new_registration_path

    expect(response).to have_http_status(:ok)
    expect(response.body).to include("Discord連携済みのサーバーに参加していない場合")
    expect(response.body).to include("Discordでログインする")
    expect(response.body).to include("プロフィールを提供してよいかDiscordに聞かれるので許可する")
    ...
    expect(Capybara.string(response.body)).to have_button("Discord でログイン")
